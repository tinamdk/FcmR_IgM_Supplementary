...
def create_comprehensive_dual_map(contacts_df, output_file='IgM_FcµR_comprehensive_map_7ytd.png'):
    ...
    try:
        ...
        # 使用gridspec来精确控制布局，增加第一个图的高度比例
        from matplotlib.gridspec import GridSpec
        gs = GridSpec(4, 2, height_ratios=[5.0, 4.0, 1.5, 3.0])  # 将第一个图的高度比例从4.0增加到5.0
        ...
        # 收集有接触的链对
        active_chain_pairs = set()
        for _, row in contacts_df.iterrows():
            chain_pair = f"m{row['IgM_chain']}-F{row['FcµR_chain']}"
            active_chain_pairs.add(chain_pair)
        ...
        # 按字母顺序排序链对，确保一致性
        sorted_chain_pairs = sorted(list(active_chain_pairs))

        # 设置等间距的y坐标，每个链对间隔10个单位
        for i, chain_pair in enumerate(sorted_chain_pairs):
            y_pos = i * 10  # 等间距，每个链对间隔10个单位
            y_positions[chain_pair] = y_pos
        ...
        for _, row in contacts_df.iterrows():
            chain_pair = f"m{row['IgM_chain']}-F{row['FcµR_chain']}"
            if chain_pair in y_positions:  # 只绘制有对应y坐标的链对
...
def main():
    ...
    # 配置参数
    PDB_FILE = "source_file/8BPE.pdb"
    IgM_CHAINS = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'K', 'L', 'J']
    FcµR_CHAINS = ['R', 'S', 'P', 'Q', 'I', 'M', 'N', 'O']
    ...
    if contacts_df is not None and not contacts_df.empty:
        # 创建综合性图谱
        create_comprehensive_dual_map(contacts_df, 'IgM_FcµR_comprehensive_map_8bpe.png')
