...
THRESHOLD = 0.0                  # 只显示平均比例 > 0.6 的残基对
OUTPUT_IMAGE_PNG = "Extended Data Figure 1.FcμR‑D1-Fcμ residue contact frequency heatmap(>0.0).png"
OUTPUT_IMAGE_PDF = "Extended Data Figure 1.FcμR‑D1-Fcμ residue contact frequency heatmap(>0.0).pdf"
...
def main():
    ...
    fig, ax = plt.subplots(figsize=(20, 16), dpi=1200)
    mask = df.isna()
    ...
    # 坐标轴标签
    ax.set_xlabel("Immunoglobulin heavy constant μ residues", fontsize=20, fontweight='bold')
    ax.set_ylabel("FcμR-D1 residues", fontsize=20, fontweight='bold')
    ax.set_title(f"FcμR-D1-Fcμ contact frequency (side-chain, distance ≤ {DIST_CUTOFF} Å)\n"
                 f"All pairs with average proportion > {THRESHOLD} are shown",
