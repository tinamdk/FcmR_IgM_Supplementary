...
def main():
    ...
    if contacts_df is not None and not contacts_df.empty:
        # 创建综合性图谱
        create_comprehensive_dual_map(contacts_df, 'IgM_FcµR_comprehensive_map_8bpg.png')

        # 保存详细数据
        contacts_df.to_csv('IgM_FcµR_detailed_interactions_8bpg.csv', index=False)
        print("详细相互作用数据已保存")
