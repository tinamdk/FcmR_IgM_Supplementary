...
THRESHOLD = 0.6                  # 只显示平均比例 > 0.6 的残基对
OUTPUT_IMAGE_PNG = "Contact_heatmaps9_0.6_include_J_SC.png"
OUTPUT_IMAGE_PDF = "Contact_heatmaps9_0.6_include_J_SC.pdf"
OUTPUT_PER_PDB_CSV = "Contact_heatmaps9_0.6_include_J_SC_manuscript2_Contact_proportions_per_PDB.csv"
...
def main():
    ...
    all_rows = []
    for pdb, frac_dict in per_pdb_fractions.items():
        for (f_res, i_res), prop in frac_dict.items():
            all_rows.append({'PDB': pdb, 'FcμR_residue': f_res, 'IgM_residue': i_res, 'proportion': prop})
    df_per_pdb = pd.DataFrame(all_rows)
    df_per_pdb.to_csv(OUTPUT_PER_PDB_CSV, index=False)
    print(f"Saved per-PDB proportions to: {OUTPUT_PER_PDB_CSV}")
    ...
    # 设置 vmin=0.6, vmax=1.0 可以增强高值对比，但会导致低于0.6的单元格不显示（已mask），所以不需要
    # 如果希望颜色从0.6开始渐变，可以设置 norm=plt.Normalize(0.6, 1.0)
    ...
    ax.set_title(f"FcμR-D1-Fcμ contact frequency (side-chain, distance ≤ {DIST_CUTOFF} Å)\n"
                 f"Only pairs with average proportion > {THRESHOLD} are shown",
    ...
    ax.set_yticklabels(fc_residues, rotation=0, fontsize=10)
