...
def create_comprehensive_dual_map(contacts_df, output_file='sIgM_FcµR_comprehensive_map_7ysg.png'):
    ...
    try:
        ...
        # 设置所有子图的边框样式
        for ax in [ax_main, ax_heatmap, ax_sIgM, ax_FcµR, ax_legend]:  # 移除了ax_contact_type
        ...
        if not sIgM_stats.empty:
            ...
            # 增加横坐标标题与图的距离为3倍
            ax_sIgM.set_xlabel('Number of Contacts', fontsize=16, fontweight='bold', labelpad=20)  # labelpad从默认15增加到45
...
def main():
    ...
    if contacts_df is not None and not contacts_df.empty:
        # 创建综合性图谱
        create_comprehensive_dual_map(contacts_df, 'sIgM_FcµR_comprehensive_map_7ysg.png')
