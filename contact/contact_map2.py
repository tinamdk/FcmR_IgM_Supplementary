...
def create_comprehensive_dual_map(contacts_df, output_file='sIgM_FcµR_comprehensive_map.png'):
    ...
    try:
        ...
        gs = GridSpec(4, 2, height_ratios=[4.0, 4.0, 3.0, 1.5])  # 图3和图4高度从3.0, 2.5增加到4.0, 4.0
        ...
        ax_main.set_ylabel('Chain Pairs (sIgM-FcµR)', fontsize=16, fontweight='bold')
        ...
        # 设置坐标轴范围
        if len(y_ticks) > 0:
            ax_main.set_ylim(y_ticks[0] - 4, y_ticks[-1] + 4)
        ...
        try:
            ...
            if not pivot_data.empty:
                ...
                ax_heatmap.set_xlabel('FcµR Residue', fontsize=16, fontweight='bold', labelpad=20)
                ax_heatmap.set_ylabel('sIgM Residue', fontsize=16, fontweight='bold')
                ...
                if len(sIgM_residues) > 0:
                    ...
                    ax_heatmap.set_yticklabels([sIgM_residues[i] for i in y_ticks], fontsize=12)
        ...
        if not sIgM_stats.empty:
            ...
            ax_sIgM.set_xlim(0, max_count * 3.5)
            ...
            ax_sIgM.set_yticklabels(sIgM_labels, fontsize=14)
            ax_sIgM.set_xlabel('Number of Contacts', fontsize=16, fontweight='bold')
            ax_sIgM.set_ylabel('sIgM Residue', fontsize=16, fontweight='bold')
            ...
            for i, (_, row) in enumerate(sIgM_stats.iterrows()):
                # 将数字标记放在条形图右侧外部
                ax_sIgM.text(row['contact_count'] + max_count * 0.02, i,
                             f"{row['contact_count']}",
                             va='center', fontsize=13, fontweight='bold')

                # 将Partners信息放在数字后面，同一行
                if row['docking_partners']:
                    # 计算Partners文本的起始位置（在数字后面）
                    partners_start_x = row['contact_count'] + max_count * 0.1
        ...
        if not FcµR_stats.empty:
            ...
            # 大幅增加x轴范围，为Partners信息留出足够空间
            ax_FcµR.set_xlim(0, max_count * 3.5)
            ...
            ax_FcµR.set_xlabel('Number of Contacts', fontsize=16, fontweight='bold')
            ...
            for i, (_, row) in enumerate(FcµR_stats.iterrows()):
                # 将数字标记放在条形图右侧外部
                ax_FcµR.text(row['contact_count'] + max_count * 0.02, i,
                             f"{row['contact_count']}",
                             va='center', fontsize=13, fontweight='bold')
